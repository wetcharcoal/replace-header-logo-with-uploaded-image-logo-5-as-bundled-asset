...
// Minimal type for file references (not in backend interface)
...
export class StorageClient {
  ...
  public async putFile(
    ...
    await (
      this.actor as unknown as {
        registerFileReference: (p: string, h: string) => Promise<void>;
      }
    ).registerFileReference(path, hashString);
  ...
  public async getDirectURL(path: string): Promise<string> {
    ...
    const fileReference = await (
      this.actor as unknown as {
        getFileReference: (p: string) => Promise<FileReference>;
      }
